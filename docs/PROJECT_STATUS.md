...
- Status: Implemented on current branch
- Root cause:
  - OpenAI images.edit API can return either `b64_json` or `url` values depending on configuration and model. The previous implementation only parsed `b64_json`, causing generations that returned `url` to fail with `AI_PROVIDER_ERROR`.
  - When the AI service returned a successful result, the NestJS backend worker marked tasks `COMPLETED` even if the generated image URL was empty, resulting in the frontend showing a success state with a default "No image available" placeholder.
- Fixes applied:
  - **AI Service:** Modified `openai_image_support.py` and `openai_image_provider.py` to be asynchronous, retrieve and parse both `b64_json` and `url` values from the OpenAI API response, fetch/download the URL image bytes when returned, and write them to the configured object storage (MinIO/S3 or local).
