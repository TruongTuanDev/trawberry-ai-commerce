...
- Status: Implemented on current branch
- What shipped:
  - additive analytics persistence for product views, search logs, and recommendation events
  - new public endpoints under `/api/public/tracking/*` and `/api/public/recommendations/*`
  - rule-based homepage and similar-product recommendation responses using the same public product card shape
  - homepage and product-detail recommendation blocks that hide silently on empty/error
  - safe storefront tracking for product views, searches, recommendation impressions, and clicks
- Safety status:
  - no checkout/order/cart/payment/shipping contract changes
  - recommendation reads degrade to empty arrays
  - tracking writes degrade to `204`
- Feature flags:
...
- Verification status:
  ...
  - Playwright recommendation smoke is added but not executed here because no app servers were started in this verification pass

## AI Try-On Generated Bytes Contract Fix - 2026-05-30

- Status: Implemented on current branch
- Root cause:
  - the active OpenAI try-on path returned `b64_json`, which `ai-service` parsed into bytes successfully
  - `backend-nest` still treated the internal AI response as URL-first and attempted to redownload the generated image before storage, causing production failures with `RESULT_IMAGE_UPLOAD_FAILED`
- Fix applied:
  - `ai-service` try-on responses now carry `imageBase64` for generated result images and do not own final try-on object persistence
  - `backend-nest` AI try-on worker now consumes the returned bytes directly, uploads them to bucket `ai-try-on` using canonical keys like `openai/<taskId>/1.png`, and only then writes task completion fields
  - backend keeps URL fallback support for providers that genuinely return a downloadable URL, but the OpenAI `gpt-image-1` path no longer depends on it
  - missing payloads now fail with `OPENAI_RESULT_IMAGE_MISSING`
- Verification status:
