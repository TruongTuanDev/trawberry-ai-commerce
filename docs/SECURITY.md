...
## Auth Hardening 2 Update

- Auth endpoints now have request throttling with generic `429` responses.
- Credentialed CORS now uses an explicit allowlist via `CORS_ALLOWED_ORIGINS`.
- Cookie config still defaults to `httpOnly + SameSite=lax`; `SameSite=none` is only honored with `Secure=true`.
- Seller pending/rejected state now uses explicit backend next-step metadata instead of UI guesswork.
