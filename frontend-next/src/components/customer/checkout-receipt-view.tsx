  ...
  return (
    ...
    >
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--muted)]">
        {t("customer.receipt.marketplaceReceipt")}
      </p>
      <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1
            className="font-[family-name:var(--font-mono-app)] text-4xl font-bold text-[var(--foreground)]"
            data-testid="receipt-checkout-code"
          >
        ...
        <div className="text-right">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--muted)]">
            {t("customer.receipt.grandTotal")}
          </p>
          <p
            className="mt-1 text-2xl font-bold text-[var(--foreground)]"
            data-testid="receipt-grand-total"
          >
            {receipt.grandTotal}
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <Metric label={t("customer.receipt.status")} value={receipt.status} />
        <Metric
          label={t("customer.receipt.orders")}
          value={String(receipt.orders.length)}
        />
        <Metric label={t("customer.receipt.customer")} value={receipt.customer.name} />
      </div>
      <div className="mt-6 grid gap-4" data-testid="receipt-child-orders">
        {receipt.orders.map((order) => (
          <article
            ...
            data-testid="receipt-order-card"
          >
            ...
            <div className="mt-4 space-y-3">
              {order.items.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-[56px_1fr_auto] gap-3 rounded-[1rem] border border-[var(--border)] bg-[var(--panel)] p-3"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={item.productImageSnapshot ?? "https://placehold.co/120x120?text=No+Image"}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-[var(--muted)]">
                      {item.variantNameSnapshot ?? t("customer.receipt.defaultVariant")} x{" "}
                      {item.quantity}
                    </p>
                  </div>
                  <p className="text-sm font-semibold text-[var(--foreground)]">
            ...
            <PaymentDetailsPanel
              ...
              title={t("customer.receipt.payShopDirectly", { shopName: order.shopName })}
              className="mt-4"
              role="customer"
            />
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm font-semibold text-[var(--foreground)]">
                {t("customer.receipt.orderTotal", { amount: order.totalAmount })}
              </p>
              <div className="flex flex-wrap gap-3">
                <Link
                  href={`/customer/returns?orderId=${encodeURIComponent(order.orderId)}`}
                  className="public-button-secondary px-4 py-2 text-sm"
                  data-testid="receipt-open-return-link"
                >
                  {t("customer.receipt.requestReturnRefund")}
                </Link>
                <Link
              ...
              <div className="mt-4 rounded-[1.25rem] border border-[var(--border)] bg-[var(--panel)] p-4">
                ...
                <div className="mt-3 space-y-2">
                  {order.returnRefundCases.map((entry) => (
                    <Link
                      key={entry.id}
                      href={`/customer/returns/${entry.id}`}
                      className="block rounded-xl border border-[var(--border)] bg-white px-4 py-3 text-sm"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <span className="font-semibold text-[var(--foreground)]">
                          {labelForReturnType(entry.type, locale)}
                        </span>
                        <span className="text-[var(--muted)]">
                          {labelForReturnStatus(entry.status, locale)}
                        </span>
                      </div>
                    </Link>
                  ))}
      ...
      {receipt.supportCases.length ? (
        <div
          className="mt-6 rounded-[1.5rem] border border-[var(--border)] bg-[var(--panel)] p-5"
          data-testid="receipt-support-summary"
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--muted)]">
              ...
              <h2 className="mt-2 text-lg font-bold text-[var(--foreground)]">
                {t("customer.support.caseCount", {
                  count: receipt.supportCases.length,
                })}
              </h2>
            </div>
          </div>
            ...
            {receipt.supportCases.map((supportCase) => (
              ...
              >
                ...
                <p className="mt-2 text-xs uppercase tracking-[0.16em] text-[var(--muted)]">
                  {t(`customer.support.issueTypes.${supportCase.issueType}`)}
                </p>
              </article>
            ))}
          </div>
