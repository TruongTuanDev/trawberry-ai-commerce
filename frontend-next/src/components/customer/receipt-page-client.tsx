...
export function ReceiptPageClient({ checkoutCode }: { checkoutCode: string }) {
  ...
  const load = async (nextPhone: string) => {
    if (!nextPhone.trim()) {
      setError(t("customer.receipt.phoneRequired"));
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      setError(err instanceof Error ? err.message : t("customer.receipt.loadFailed"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="px-4 py-8 sm:px-6">
        <div className="mx-auto max-w-7xl space-y-6">
          <div className="flex flex-wrap gap-3">
            <Link href="/products" className="public-button-secondary px-4 py-2 text-sm">{t("customer.receipt.marketplace")}</Link>
            <Link href="/customer/orders" className="public-button-secondary px-4 py-2 text-sm">{t("customer.receipt.myOrders")}</Link>
          </div>
          {!receipt ? (
            <section className="card-panel rounded-[2rem] px-6 py-8">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--muted)]">{t("customer.receipt.lookupEyebrow")}</p>
              <h1 className="mt-3 font-[family-name:var(--font-mono-app)] text-3xl font-bold text-[var(--foreground)]">{checkoutCode}</h1>
              <div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto]">
                <input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder={t("customer.receipt.phonePlaceholder")} className="public-input" data-testid="receipt-phone" />
                <button type="button" onClick={() => void load(phone)} disabled={loading} className="public-button-primary px-5 py-3 text-sm" data-testid="receipt-load">
                  {loading ? t("common.loading") : t("customer.receipt.loadButton")}
                </button>
              </div>
