    ...
    <PublicShell>
      <main className="px-4 py-8 sm:px-6 sm:py-10">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <section className="card-panel rounded-[2rem] px-6 py-6 sm:px-8" data-testid="tracked-order-page">
              ...
              <section className="space-y-6">
                <section className="card-panel rounded-[2rem] px-6 py-8 sm:px-8">
                  ...
                  <div className="mt-6 grid gap-4 rounded-[1.5rem] border border-[var(--border)] bg-white p-5 md:grid-cols-2">
                    ...
                    <TextMetric label="Email" value={order.customer.email ?? "Not provided"} />
                    <TextMetric label="Address" value={order.customer.address} />
                  </div>
                    ...
                    {order.paymentLogs.length ? (
                      order.paymentLogs.map((log) => (
                        <article key={log.id} className="rounded-[1.5rem] border border-[var(--border)] bg-white p-4">
                          ...
                          <p className="mt-2 text-sm text-[var(--foreground)]">{log.note ?? "No note attached."}</p>
                        </article>
                      ))
                    ) : (
                      <p className="text-sm text-[var(--muted)]">No payment activity recorded yet.</p>
                    )}
                  </div>
                </section>
              </section>
