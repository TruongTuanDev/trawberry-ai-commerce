    ...
    <header className="sticky top-0 z-30 border-b border-[var(--border)]/70 bg-[rgba(249,243,234,0.82)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
        <Link href="/" className="flex items-center gap-3" data-testid="public-logo">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--muted)]">Marketplace</p>
            <p className="font-[family-name:var(--font-mono-app)] text-lg font-bold text-[var(--foreground)]">Strawberry AI Commerce</p>
          </div>
        </Link>

        <nav className="hidden items-center gap-2 md:flex" aria-label="Public navigation" data-testid="public-nav">
          {links.map((link) => {
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  active ? "bg-[var(--foreground)] text-white" : "text-[var(--foreground)] hover:bg-white",
                )}
              >
                {link.label}
              </Link>
        ...
        <Link href="/products" className="public-button-primary px-4 py-2 text-sm shadow-[0_14px_28px_rgba(182,49,75,0.2)]">
          Shop now
        </Link>
      </div>
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                active ? "bg-[var(--foreground)] text-white" : "bg-white text-[var(--foreground)]",
              )}
            >
              {link.label}
            </Link>
